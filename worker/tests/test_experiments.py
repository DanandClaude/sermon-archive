...
QUICK = {"default": CleanConfig(), "no-denoise": CleanConfig(denoise=False)}
...
def test_each_variant_writes_a_file_and_the_default_removes_hum(hummy_wav, tmp_path):
    ...
    assert [r["variant"] for r in rows] == ["original", "default", "no-denoise"]
...
def test_can_compare_transcription_quality_across_variants(hummy_wav, tmp_path):
    ...
    assert "confidence" in table and "no-denoise" in table
...
def test_the_built_in_variants_include_a_50_hz_option():
    assert VARIANTS["hum-50hz"].hum_hz == (50.0, 100.0, 150.0)
    assert VARIANTS["no-denoise"].denoise is False
