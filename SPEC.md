...
*Built as (2026-09-18):* the worker claims jobs from a Postgres `jobs` table (the app enqueues in the same transaction as the upload finishing) rather than pg-boss. Transcription is faster-whisper on the worker's machine, so the spec's manual chunking (only needed for hosted APIs) is not used. SRT, VTT and text are rendered from the stored word timings by one TypeScript function instead of being stored. Finished transcripts leave a sermon at `analyzing`; Phase 3 picks them up. A worker heartbeat lets the app say when processing is paused. Transcription has two engines, MLX on an Apple GPU and faster-whisper on a CPU. Tuned on two real clips: the worker transcribes the original audio by default, adds hum notches only when hum is detected, and leaves denoising, click removal and declipping off, because cleanup did not improve transcription and denoising slightly lowered it; the cleaned copy is mainly for listening. `sermon_worker.experiments` compares variants. Long segments are split at sentence ends. Not built: resuming a transcription interrupted part way, and a container image.
...
*Built as (2026-09-18):* the analysis runs in the Python worker as an `analyze` job, with a fake analyzer for development and an Anthropic one (default model `claude-sonnet-5`, structured JSON output) enabled by `ANALYZER=anthropic`; the Anthropic path is tested only against a stub client, not the live API. The canon table is generated from the Project Gutenberg KJV by `scripts/build-canon.ts` (only the derived counts are committed) and is shared by the app and the worker, with `shared/reference-cases.json` keeping the two parsers in step. Detection is a deterministic parser, then the model judges, corrects and adds passages (each addition must quote the transcript so it can be timed), then canon validation and merging. Approval names the file at that moment (`_2`, `_3` on collision) and freezes it; post-approval renames are Phase 4. Edits are allowed only once a sermon is waiting for review (admins also after approval); deleted passages are soft-deleted and audited. The review screen shows both waveforms, a playhead synced with the passage list and transcript, and a confirmation before approving. Filing to a drive is not connected, and the approval card says so. Not built: editing transcript text, cross-chapter ranges, the phone-width layout of the app shell.
...
*Built as (2026-09-18):* filing and verification run in the Python worker (a `file` job queued on approval; `verification_runs` for "Verify now" and the nightly check). Decided with the owner: the church has no Google Workspace, so Drive targets are ordinary folders in one Google account each (scope `drive.file`, so the app sees only what it creates), and the shared drive and the backup must be different accounts (enforced when connecting). Providers are create-only; identical files already present are adopted and different ones stop the job. Each file is read back and its checksum compared before a sermon is `filed`; a failure leaves it `approved` with a visible reason and an admin "File again". Credentials are AES-256-GCM encrypted with `SECRETS_KEY`. The Drive provider is tested only against a stand-in service, and real Drive only runs in production mode (a development target is a local folder under `.data/targets/`). Deferred by the owner: streaming Drive files through the app (D4, Phase 6) and renaming files after approval. Also not built: repairing drifted files (an admin fixes them by hand), and re-syncing edits an admin makes after filing.

*Deployment work added (2026-09-19), before Phase 5:* a Docker install (`deploy/`: web and worker images, Caddy for HTTPS, Postgres, one-time migrations, a nightly backup service, and a smoke test), an install guide and a restore guide, a phone-width layout, and the project published under the MIT license. Database backups go to the same bucket under `db-backups/` (14 daily, 8 weekly Sunday copies, optionally encrypted) and their status shows on the Connections page.
