...
- Docker (for the install in `deploy/`): `deploy/smoke-test.sh` builds both images and runs the whole stack with a throwaway bucket and mail catcher, then takes and restores a database backup. Docker on this Mac is Colima (`colima start`), installed with Homebrew. Images: `Dockerfile` (web) and `worker/Dockerfile` (worker and backup service). `npm run build:scripts` bundles the migration and admin scripts into `dist/` for the image.
...
- Backups: `worker/src/sermon_worker/backup.py` dumps the database nightly to `db-backups/` in the bucket (14 daily, 8 weekly Sundays; only that prefix is ever listed or deleted), optionally encrypted with `BACKUP_PASSPHRASE`, and records the result in `settings.backup_last` for the Connections page. Restore refuses a non-empty database unless `--overwrite`.
